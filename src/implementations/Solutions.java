...
public class Solutions {
	...
	final String LARGESUM = "37107287533902102798797998220837590246510135740250"
	...
	public int[][] buildGrid(){
		...
		for(int i = 0; i < 20; i++) {
			for(int j = 0; j < 20; j++) {
				ret[i][j] = Integer.parseInt(GRID.substring(countStringStart, countStringEnd));
				countStringStart += 3;
				countStringEnd += 3;
			}
			countStringStart--;
	...
	public String[] buildLargeSum() {
		...
		for(int i = 0; i < 100; i++) {
			for(int j = 0; j < 50; j++) {
				actualString += LARGESUM.charAt((i*50) + j);
			}
			ret[i] = actualString;
			actualString = "";
		}
